...
export class ActionConfig extends FormApplication {
    constructor(object, options = {}) {
        ...
        let token = game.scenes.viewed.tokens?.contents[0];
        if (token) {
            try {
        ...
        let tile = game.scenes.viewed.tiles?.contents[0];
        if (tile) {
            try {
    ...
    static async updateSelection(selection, event) {
        ...
        if (this.options.parent) {
            if (game.scenes.viewed.id != this.options.parent.object.parent.id)
