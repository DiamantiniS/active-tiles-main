...
export class MonksActiveTiles {
    ...
    static findTileTriggers = async () => {
        ...
        for (let tile of game.scenes.viewed.tiles.contents) {
            let triggerData = tile.flags["monks-active-tiles"];
            ...
            if (l.id) {
                ...
                if (!(dest instanceof Document)) {
                    ...
                    if (uuid.startsWith("Tile."))
                        uuid = "Scene." + (l.sceneId || game.scenes.viewed.id) + "." + uuid;
                    if (uuid.length == 16)
                        uuid = "Scene." + (l.sceneId || game.scenes.viewed.id) + ".Tile." + uuid;
                ...
                location[i] = {
                    ...
                    y: y,
                    scale: l.scale,
                    scene: l.sceneId || game.scenes.viewed.id
                };
            }
        }
    ...
    static async locationName(location) {
        ...
        let sceneId = _location.sceneId || game.scenes.viewed.id;
        if (_location.id) {
            if (_location?.id == 'previous' || _location?.id == 'current')
                ...
                if (uuid.startsWith("Tile."))
                    uuid = "Scene." + (_location.sceneId || game.scenes.viewed.id) + "." + uuid;
                if (uuid.length == 16)
                    uuid = "Scene." + (_location.sceneId || game.scenes.viewed.id) + ".Tile." + uuid;
        ...
        return `${(scene?.id != game.scenes.viewed.id ? 'Scene: ' + scene?.name + ', ' : '')}${name}`;
    }
    ...
    static selectClick = function (event) {
        ...
        if (waitingType == 'location' || waitingType == 'either' || waitingType == 'position') {
            ...
            if (restrict && !restrict(game.scenes.viewed)) {
                ui.notifications.error(i18n("MonksActiveTiles.msg.invalid-location"));
                return;
            }
            let pos = canvas.activeLayer.toLocal(event);
            let update = { x: parseInt(pos.x), y: parseInt(pos.y), sceneId: (game.scenes.viewed.id != MonksActiveTiles.waitingInput.options.parent.object.parent.id ? game.scenes.viewed.id : null) };
    ...
    static async init() {
        ...
        let onDropData = async function (wrapper, ...args) {
            ...
            if (data.data) {
                ...
                return TileDocument.create(tileData, { parent: game.scenes.viewed }).then((tile) => {
    ...
    static async _onClickTileLink(event) {
        ...
        if (!uuid.startsWith("Scene"))
            uuid = `Scene.${game.scenes.viewed.id}.${!uuid.startsWith("Tile") ? "Tile." : ""}${uuid}`;
        ...
        switch (data.action) {
            ...
            case 'switchview': {
                if (data.users.find(u => u == game.user.id) != undefined) {
                    //let oldSize = game.scenes.viewed.dimensions.size;
                    //let oldPos = game.scenes.viewed._viewPosition;
                    let offset = { dx: (game.scenes.viewed._viewPosition.x - data.oldpos?.x), dy: (game.scenes.viewed._viewPosition.y - data.oldpos?.y) };
                    let scene = game.scenes.get(data.sceneid);
                    if (!canvas.loading && game.scenes.viewed.id != scene.id) {
            ...
            case 'playsound': {
                if (data.users.includes(game.user.id) && (data.sceneid == null || data.sceneid == game.scenes.viewed.id)) {
                    let tile = await fromUuid(data.tileid);
                    if (tile) {
            ...
            case 'offsetpan': {
                if (data.userId == game.user.id) {
                    if (data.animatepan)
                        canvas.animatePan({ x: game.scenes.viewed._viewPosition.x - data.x, y: game.scenes.viewed._viewPosition.y - data.y });
                    else
                        canvas.pan({ x: game.scenes.viewed._viewPosition.x - data.x, y: game.scenes.viewed._viewPosition.y - data.y });
                }
            } break;
            case 'fade': {
                if (data.users.includes(game.user.id)) {
            ...
            case 'transition':
                {
                    ...
                    if (entity) {
                        ...
                        if (!canView) {
                            ...
                            canView = !entity.hidden && entity.parent.id == game.scenes.viewed.id && (!game.scenes.viewed.tokenVision || (token && canvas.visibility.testVisibility({ x: token.x, y: token.y }, { tolerance: 1, object: entity })));
                        }
                        if (canView)
    ...
    static setupTile() {
        ...
        TileDocument.prototype.preloadScene = function () {
            ...
            for (let action of actions) {
                if (action.action == 'teleport' && action.data.location.sceneId && action.data.location.sceneId != game.scenes.viewed.id) {
        ...
        TileDocument.prototype.getHistory = function (tokenid) {
            ...
            for (let [k, v] of Object.entries(tileHistory)) {
                if (tokenid == undefined || tokenid == k) {
                    ...
                    let token = game.scenes.viewed.tokens.find(t => t.id == k);
    ...
    static changeActive(event) {
        ...
        return game.scenes.viewed.updateEmbeddedDocuments(this.object.document.documentName, updates);
    }
...
Hooks.on("updateCombat", async function (combat, delta) {
    if (combat.started && game.user.isGM && combat.scene) {
        let sceneTiles = combat.scene.id == game.scenes.viewed.id ? MonksActiveTiles.tileTriggerCache.combat : combat.scene.tiles;
...
Hooks.on("dropCanvasData", async (canvas, data, options, test) => {
    if (data.type == 'Item' && setting('drop-item')) {
        ...
        let size = game.scenes.viewed.dimensions.size * setting("drop-item-size");
        ...
        await cls.create(td, { parent: game.scenes.viewed });
        ...
        let size = game.scenes.viewed.dimensions.size;
        let dest = { x: data.x - (size / 2), y: data.y - (size / 2) };
...
Hooks.on("updateScene", async (scene, data, options) => {
    if (data.environment?.darknessLevel != undefined) {
        let sceneTile = scene.id == game.scenes.viewed.id ? MonksActiveTiles.tileTriggerCache.darkness : scene?.tiles;
...
Hooks.on("lightingRefresh", async (lightinEffect) => {
    let tiles = MonksActiveTiles.tileTriggerCache.lighting.map(tile => {
        ...
        //Trigger this Tile
        return { tile, args: { tokens: [], method: 'lighting', options: { darkness: game.scenes.viewed.environment?.darknessLevel } } };
    }).filter(t => !!t).sort((a, b) => (b.tile.sort ?? b.tile.z) - (a.tile.sort ?? a.tile.z));
    for (let t of tiles) {
        let triggerResult = await t.tile.trigger(t.args);
        if (triggerResult?.stoptriggers)
