        ...
        return {
                ...
                fn: async (args = {}) => {
                    ...
                    if (action.data.location?.id == "players") {
                        ...
                        for (let i = 0; i < dests.length; i++) {
                            let dest = dests[i];
                            if (dest.scene != undefined && dest.scene != game.scenes.viewed.id)
                                return;

                            dest.duration = (action.data?.duration ?? 1) * 1000;

                            dest.x = parseInt(await getValue(dest.x, args, dest, { prop: game.scenes.viewed._viewPosition.x, explicit: true }));
                            dest.y = parseInt(await getValue(dest.y, args, dest, { prop: game.scenes.viewed._viewPosition.y, explicit: true }));
                ...
                fn: async (args = {}) => {
                    ...
                    for (let tokendoc of entities) {
                        ...
                        if (samescene) {
                            ...
                            //await tokendoc.update(/* TODO: verifica se è un embedded document */ { x: newPos.x, y: newPos.y }, { bypass: true, animate: false, teleport: true });
                        } else {
                            result.tokens = [];
                            ...
                            if (newtoken) {
                                ...
                                //await newtoken.update(/* TODO: verifica se è un embedded document */ (action.data.preservesettings ? { x: newPos.x, y: newPos.y, hidden: tokendoc.hidden } : td), { bypass: true, animate: false, teleport: true });
                            } else {
                            ...
                            if (anim && entity._matt_locations) {
                                let fn = async (entity) => {
                                    if (entity._matt_locations){
                                        ...
                                        await entity.update(/* TODO: verifica se è un embedded document */ pt, { animate: true, animation: { duration } });
                                        let anim = CanvasAnimation.getAnimation(entity._object.animationName);
                                        if (anim && entity._matt_locations)
                ...
                fn: async (args = {}) => {
                    ...
                    if (entities && entities.length > 0) {
                                                ...
                                                if (entDest) {
                                                    if (dest.dest instanceof TileDocument) {
                                                        if (action.data.position == "center") {
                                                            ...
                                                            // Find a random location within this Tile
                                                            let midX = ((game.scenes.viewed.dimensions.size * Math.abs(entity.width ?? 1)) / 2);
                                                            let midY = ((game.scenes.viewed.dimensions.size * Math.abs(entity.height ?? 1)) / 2);
                                                            entDest.x = (dest.dest.x + midX) + Math.floor((Math.random() * (Math.abs(dest.dest.width) - (Math.abs(actor.prototypeToken.width) * game.scenes.viewed.dimensions.size))));
                                                            entDest.y = (dest.dest.y + midY) + Math.floor((Math.random() * (Math.abs(dest.dest.height) - (Math.abs(actor.prototypeToken.height) * game.scenes.viewed.dimensions.size))));
                                                        }
                                                    }
                                                    let data = {
                                                        x: entDest.x,
                                    ...
                                    if (dest.dest instanceof TileDocument) {
                                        ...
                                        if (dest.dest instanceof TileDocument) {
                                            if (action.data.position == "center") {
                                                ...
                                                // Find a random location within this Tile
                                                let midX = ((game.scenes.viewed.dimensions.size * Math.abs(entity.prototypeToken.width)) / 2);
                                                let midY = ((game.scenes.viewed.dimensions.size * Math.abs(entity.prototypeToken.height)) / 2);
                                                entDest.x = (dest.dest.x + midX) + Math.floor((Math.random() * (Math.abs(dest.dest.width) - (Math.abs(entity.prototypeToken.width) * game.scenes.viewed.dimensions.size))));
                                                entDest.y = (dest.dest.y + midY) + Math.floor((Math.random() * (Math.abs(dest.dest.height) - (Math.abs(entity.prototypeToken.height) * game.scenes.viewed.dimensions.size))));
                                            }
                                        }
                                    } else {
                                        if (entDest.x) {
                        ...
                        for (let ad of actors) {
                            ...
                            //if (td.hidden)
                            //    tkn.update(/* TODO: verifica se è un embedded document */ { hidden: true });
            ...
            'playsound': {
                ...
                fn: async (args = {}) => {
                    ...
                    if (showUsers.includes(game.user.id)) {
                        if (action.data.scenerestrict && tile.parent.id != game.scenes.viewed.id)
                            return;
                        ...
                        if (typeof action.data.text == "string" && (action.data.text.startsWith('/') || action.data.text.startsWith('[[/'))) {
                            ...
                            if (action.data.chatbubble !== "false") {
                                if (tkn instanceof Token) {
                                    ...
                                    if (su.includes(game.user.id) && game.scenes.viewed.id == tkn.document.parent.id) {
                        ...
                        if (entity.object && entity.object.isVideo) {
                            ...
                            if (currentAction != newAction) {
                                if (action.data.animatefor === 'everyone') {
                                    ...
                                    await entity.update(/* TODO: verifica se è un embedded document */ { video }, {
                                        diff: false,
                ...
                fn: async (args = {}) => {
                    ...
                    for (let entity of entities) {
                        ...
                        if (action.data.permission != "true" && action.data.permission != "false") {
                            ...
                            await entity.update(/* TODO: verifica se è un embedded document */ { ownership });
                        }
            ...
            'permissions': {
                ...
                fn: async (args = {}) => {
                    ...
                    for (let entity of entities) {
                        ...
                        await entity.update(/* TODO: verifica se è un embedded document */ { ownership: perms }, { diff: false, render: true, recursive: false, noHook: true });
            ...
            'scenebackground': {
                ...
                fn: async (args = {}) => {
                    ...
                    for (let scene of entities) {
                        ...
                        await scene.update(/* TODO: verifica se è un embedded document */ { "background.src": img });
                    }
                },
                content: async (trigger, action) => {
                    let ctrl = trigger.ctrls.find(c => c.id == "sceneid");
            ...
            'addtocombat': {
                ...
                fn: async (args = {}) => {
                    ...
                    if (!combat) {
                        ...
                        combat = await cls.create({ scene: game.scenes.viewed.id, active: true });
                    }
                        ...
                        for (let entity of entities) {
                            ...
                            if (entityObject?._transition || CanvasAnimation.getAnimation(animationName)) {
                                ...
                                if (entityObject && entityObject._transition_time < new Date().getTime()) {
                                    ...
                                    await entity.update(/* TODO: verifica se è un embedded document */ { texture: { src: entity._object._transition_to } });
                            ...
                            if (action.data.transition == "none") {
                                if (entity._images[position - 1]) {
                                    await entity.update(/* TODO: verifica se è un embedded document */ { texture: { src: entity._images[position - 1] }, 'flags.monks-active-tiles.fileindex': position - 1 });
                                ...
                                entity._transitionPromise.then(() => {
                                    entity._transitionPromise = entity.update(/* TODO: verifica se è un embedded document */ { texture: { src: transData.img }, 'flags.monks-active-tiles.fileindex': transData.position });
                                });
                                */
                                let loop = action.data.loop || 1;
                                ...
                                const runAction = async (data, result) => {
                                    ...
                                    if (loop > 0) {
                                        ...
                                        await entity.update(/* TODO: verifica se è un embedded document */ { texture: { src: result }, 'flags.monks-active-tiles.fileindex': data.position });
                                    }
                                }
            ...
            'scenelighting': {
                ...
                fn: async (args = {}) => {
                    ...
                    tile.parent.update(/* TODO: verifica se è un embedded document */ { darkness: action.data.darkness }, { animateDarkness: (action.data.speed * 1000) });
                },
                content: async (trigger, action) => {
                ...
                fn: async (args = {}) => {
                    ...
                    for (let entity of entities) {
                        if (entity instanceof JournalEntry) {
                            ...
                            await page.update(/* TODO: verifica se è un embedded document */ { text: { content: content } });
                        }
                    }
                },
                content: async (trigger, action) => {
